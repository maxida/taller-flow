...
import { Search, Wrench, FileText, AlertTriangle, CheckCircle, Clock, ChevronRight, Loader2, Package, Plus, Minus, Trash2, Camera, DollarSign, Calculator, Percent } from 'lucide-react';
...
export default function GestionOT() {
  ...
  // Efecto: Sincroniza la suma de la lista de repuestos con el recuadro manual de forma inteligente
  useEffect(() => {
    const sumaAutomatica = repuestosSeleccionados.reduce((acc, rep) => acc + (rep.cantidadUsada * (rep.precioUnitario || 0)), 0);
    // Solo sobreescribimos el recuadro si la suma automática es mayor a 0 (para no borrar lo que el mecánico tipee a mano)
    if (sumaAutomatica > 0) {
  ...
  // Al seleccionar una OT, limpiamos todos los campos
  ...
  const handleFinalizar = async (e) => {
    ...
    try {
      ...
      // 2. ACTUALIZAR LA OT CON DATOS FINANCIEROS
      ...
      await updateDoc(otRef, {
        ...
        // Datos Financieros KPIs
        costoManoObra: costoManoObra,
        costoRepuestos: costoRepuestos, // Guardamos el valor maestro (sea manual o auto-sumado)
        descuento: descuento,
  ...
  return (
    <div className="max-w-7xl mx-auto h-[calc(100vh-8rem)] flex flex-col">
      <div className="mb-6">
        ...
        <p className="mt-2 text-slate-500">Completá el informe, registrá repuestos, costos y cerrá la OT.</p>
      </div>
              ...
              <form onSubmit={handleFinalizar} className="space-y-8">
                ...
                <div className="bg-slate-50 p-6 rounded-xl border border-slate-200">
                  <h4 className="flex items-center gap-2 font-bold text-slate-800 mb-4">
                    <Package className="text-blue-600" size={20} /> Repuestos Utilizados y Precios
                  </h4>
                  ...
                  {repuestosSeleccionados.length > 0 && (
                    <div className="bg-white border border-slate-200 rounded-lg overflow-hidden">
                      {repuestosSeleccionados.map((rep) => (
                        <div key={rep.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-3 border-b last:border-0 hover:bg-slate-50 gap-3">
                          ...
                          <div className="flex items-center gap-3">
                            <div className="relative">
                              ...
                              <input 
                                ...
                                className="w-28 pl-7 pr-3 py-1.5 text-right rounded-md border border-slate-300 text-sm focus:ring-2 focus:ring-blue-500 outline-none font-medium"
                ...
                {/* RESUMEN FINANCIERO A 3 COLUMNAS */}
                <div className="bg-slate-800 rounded-xl p-6 text-white shadow-md">
                  <h4 className="flex items-center gap-2 font-bold text-slate-200 mb-6 border-b border-slate-700 pb-3">
                    <Calculator className="text-blue-400" size={20} /> Resumen de Costos y Facturación
                  </h4>
                  
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    {/* 1. Costo Repuestos (Input Maestro) */}
                    <div>
                      <label className="block text-xs text-slate-400 uppercase tracking-wider mb-2 font-bold">Costo Repuestos ($)</label>
                      <div className="relative">
                        ...
                        <input 
                          ...
                          className="w-full pl-10 pr-4 py-3 rounded-lg border border-slate-600 bg-slate-900 text-white focus:ring-2 focus:ring-blue-500 outline-none font-medium transition-colors"
                    ...
                    <div>
                      <label className="block text-xs text-slate-400 uppercase tracking-wider mb-2 font-bold">Costo Mano Obra ($)</label>
                      <div className="relative">
                    ...
                    {/* 3. Descuento Global */}
                    <div>
                      <label className="block text-xs text-slate-400 uppercase tracking-wider mb-2 font-bold">Descuento Global (%)</label>
                      <div className="relative">
                  ...
                  <div className="bg-slate-900 rounded-lg p-5 space-y-3">
                    ...
                    <div className="pt-3 mt-3 border-t border-slate-700 flex justify-between items-center">
                      ...
                      <span className="text-2xl font-black text-blue-400">${formatearMiles(totalFinal)}</span>
                    </div>
                  </div>
                </div>

                <div className="flex justify-end">
                  <button type="submit" disabled={isSubmitting} className="bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-600 text-white font-bold py-4 px-8 rounded-xl shadow-lg transition-all flex items-center gap-3 w-full md:w-auto justify-center">
                    {isSubmitting ? <><Loader2 className="animate-spin" size={24} /> Registrando Facturación...</> : <><CheckCircle size={24} /> Finalizar y Cerrar OT</>}
                  </button>
                </div>
              </form>
            </div>
